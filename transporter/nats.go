...
func (nt *Nats) Listen() error {
	...
	if nt.conn == nil {
		// Connect Options.
		opts := nt.setupConnOptions()

		// Connect to NATS
		conn, err := nats.Connect(nt.connUrls, opts...)
		if err != nil {
			log.Error(err)
			return err
		}
...
// TODO: make options available to be configurable
func (nt *Nats) setupConnOptions() []nats.Option {
	totalWait := 10 * time.Minute
	reconnectDelay := time.Second

	opts := append(nt.opts, nats.ReconnectWait(reconnectDelay))
	opts = append(nt.opts, nats.MaxReconnects(int(totalWait/reconnectDelay)))
...
// NewNats returns a new instance of nats transporter which will connects to the urls
// provided as parameter
func NewNats(connUrls string, opts []nats.Option) Nats {
	return Nats{
		connUrls:  connUrls,
...
// NewNatsExistingCon returns a new instance of nats transporter using an existing
// nats connection.
func NewNatsExistingCon(conn *nats.Conn) Nats {
	return Nats{
		conn:      conn,
		actions:   make(map[string]handler.ListenerAction),
		closeChan: make(chan bool),
